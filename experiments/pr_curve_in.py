...
for inx, col in enumerate(cols):
    ...
    for n in range(10):
        ...
        with open(xlnet_read_path, 'rb') as file:
            ...
            xlnet_pr_auc = roc_auc_score(xlnet_true_label, xlnet_predict_prob)
        ...
        lstm_pr_auc = roc_auc_score(lstm_true_label, lstm_predict_prob)
        ...
        rb_pr_auc = roc_auc_score(true_label, predict_label)
        rb_auc_array.append(rb_pr_auc)
    ...
    #ax.plot(xlnet_mean_recall, xlnet_mean_precision,  label=('XLNet (AUROC = {0:0.2f}'.format(np.mean(xlnet_auc_array))+'±{0:0.2f})'.format(np.std(xlnet_auc_array))))
    ax.plot(xlnet_mean_recall, xlnet_mean_precision, label='XLNet', alpha = 0.3, ls = '-.')
    ...
    ax.fill_between(xlnet_mean_recall, xlnet_tprs_lower, xlnet_tprs_upper, alpha=.1)

    lstm_mean_precision = np.mean(lstm_precision_array, axis=0)
    lstm_std_precision = np.std(lstm_precision_array, axis=0)
    #ax.plot(lstm_mean_recall, lstm_mean_precision, label=('LSTM (AUROC = {0:0.2f}'.format(np.mean(lstm_auc_array))+'±{0:0.2f})'.format(np.std(lstm_auc_array))))
    ax.plot(lstm_mean_recall, lstm_mean_precision, label='LSTM', alpha = 0.7, ls = '--')
    ...
    ax.fill_between(lstm_mean_recall, lstm_tprs_lower, lstm_tprs_upper, alpha=.1)

    rb_mean_precision = np.mean(rb_precision_array, axis=0)
    rb_std_precision = np.std(rb_precision_array, axis=0)
    #ax.plot(rb_mean_recall, rb_mean_precision, label=('Rule-based (AUROC = {0:0.2f}'.format(np.mean(rb_auc_array))+'±{0:0.2f})'.format(np.std(rb_auc_array))))
    ax.plot(rb_mean_recall, rb_mean_precision, label='Rule-based', alpha = 1.0, ls = ':')
    ...
    ax.fill_between(rb_mean_recall, rb_tprs_lower, rb_tprs_upper, alpha=.1)
